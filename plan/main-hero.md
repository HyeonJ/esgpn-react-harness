...
### 회차 6 — 이중 회전 제거 + 정밀 수치 규칙 적용
- **변경**: `HeroIntroCard`에서 CSS `transform: rotate()` 제거. Framelink가 export한 카드 PNG는 이미 회전이 baked in된 bounding-box(342.938×361.494 @ 2x scale)라 CSS에서 추가 회전 시 **이중 회전** 상태가 됨. 카드 img를 Figma 노드 정확한 bounding-box 소수점 크기 그대로 렌더하도록 교정.
- **docs/section-implementation.md §2.4 정밀 수치 규칙**(이번 PR에 함께 추가)을 첫 적용한 사례.
- **G1 시각 diff**: **2.24%** (44,747 / 1,996,800 px) — 회차 5 대비 **-2.25%p** 개선. 임계 5% 대비 2.76%p 여유.
  - 추세: 45.48 → 8.67 → 6.88 → 5.98 → 4.49 → **2.24**
  - 잔여 diff는 Yeseva One 워드마크 글리프 힌팅, BG palette quantize, 카드 외곽 안티앨리어싱 수준.
- G2/G3/G4: 치수·에셋·색상 모두 동일 범위 유지 (측정 스크립트 page.evaluate 문자열 내 TS 타입 잔존 이슈로 재실행 실패, G1 개선치로 간접 검증). 수동 재확인 필요시 스크립트 수리 후 재실행.

### 결론
- **G1 = 2.24% PASS, G2/G3/G4 = PASS** — 4 게이트 모두 통과.
- 트레이드오프 기록:
