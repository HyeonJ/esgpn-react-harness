...
## 11. v4 자율 모드 재실행 (34/43 완주)

**컨텍스트**: 이전 세션에서 plan·측정 기록은 남았으나 실제 ContactForm 컴포넌트 파일이 유실된 상태로 재진입. v4 자율 모드로 단계 3~7 연속 완주.

### v4 차단 게이트 결과
| 게이트 | 결과 | 수치 |
|---|---|---|
| **G5 시맨틱** | PASS | eslint 출력 0건 (`<form>` + `<fieldset>`/`<legend>` + `<label htmlFor>` + `<input id>` + `<button type="submit">`) |
| **G6 텍스트:이미지** | PASS | ratio 21.70 (217 text / 10 alt), raster-heavy=false |
| **G8 i18n 가능성** | PASS | JSX literal text 존재 확인 |
| **G2 치수** | PASS | 회차 2 측정과 동일 (±0) |
| **G4 색상** | PASS | 모든 토큰 spec 일치 |
| 참고 G1 시각 diff | 4.01% ref | 차단 아님 (≤15% 목표) |
| 참고 G3 에셋 | PASS (공백) | imgs=[] (에셋 0개, CSS placeholder) |
| 자동 가드 Tailwind AP | PASS | 음수 width 0, 정수 반올림 0 |
| 자동 가드 baked-in PNG | PASS | Framelink 중첩 0 |

### 구조 지표 (v1~v3 vs v4)
- **token_ratio**: `var(--color-*)` 4회(HatchedDivider) + `var(--font-family-pretendard)` 5회 / 총 magic-color 5 (#1d2623, #f3f4f5, #313c4c, #687685, rgba 0.2) + 토큰 9 → **9/14 = 0.64** ✅ (목표 ≥0.2)
- **absolute/file**: ContactForm 0, FormInput 0, Preview 0 → **0/3 = 0** ✅ (목표 ≤5)
- **semantic_score**: `<section>` + `<h1>` + `<p>` + `<form>` + `<fieldset>` + `<legend>` + `<label>` + `<button>` = **8** ✅ (목표 ≥2)

### 정적 폼 검증
- `<form onSubmit={(e) => e.preventDefault()}>` — 페이지 리로드 방지
- `<button type="submit">` + `required` 속성 (접근성 향상)
- 실제 백엔드 연결은 과제 범위 밖 (사용자 결정 §9)
