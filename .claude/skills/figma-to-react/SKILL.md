...
당신은 **메인 세션의 오케스트레이터**다. 직접 코드를 짜지 않는다. 대신 서브에이전트 워커를 스폰하고, 결과를 검증하고, **완주까지 멈추지 않는다 (v4 자율 모드).**

## 철학 (v4 — "편집 가능한 고충실도")

> North star: `docs/redefine/philosophy.md`

1. **작업 단위는 페이지가 아니라 섹션이다** — 한 섹션 = 한 워커 호출 = 한 커밋
2. **측정값을 숫자로 기록하지 않은 섹션은 미완료** — 눈대중 금지
3. **MCP 에셋 URL은 무조건 다운로드, 동적 에셋은 정적 프레임 추출**
4. **차단 게이트 = 구조 (G5/G6/G8/G2/G4). G1 시각 diff = 참고 지표 (≤15%)**
5. **사용자 개입 0** — 완주 후 검수. 3회 실패 시에도 AI 자체 판단

## 시간 측정 (v4 필수)
- Phase 2 시작 전: `date +%s` → PROGRESS.md "시작" 필드
- 각 섹션 워커 반환 시: 소요 시간(초) 기록
- 전 페이지 완주 후: 총 소요 시간 계산 → PROGRESS.md "종료" + "총 소요"
...
4. `plan/*.md`의 미완료 섹션
5. **`FIGMA_TOKEN` 환경 변수 세팅 여부** (`powershell -Command "[Environment]::GetEnvironmentVariable('FIGMA_TOKEN', 'User')"` 또는 Unix `printenv FIGMA_TOKEN`). 미세팅이면 섹션 워커 스폰 전에 사용자에게 `docs/figma-workflow.md` Phase 0 안내. Figma REST API 래퍼(`scripts/figma-rest-image.sh`)가 이 토큰으로 동작. **Framelink MCP 절대 호출 금지** (F-015 영구 폐기)
...
3. 각 섹션 + 페이지 전체 `scripts/figma-rest-image.sh <fileKey> <nodeId> figma-screenshots/{섹션명}.png --scale 2` + `figma-screenshots/{페이지명}-full.png` (Framelink 금지)
...
### 단계별 오케스트레이터 대응 (v4 자율 — 멈춤 0)

| 워커 반환 | 오케스트레이터 행동 |
|----------|------------------|
| 단계 1(리서치) 완료 | 로그 기록 → 자동으로 단계 2 진행 |
| 단계 2(계획) 완료 | 로그 기록 → 자동으로 단계 3 진행 (**승인 대기 없음**) |
| 단계 4.5 (G5~G8) 통과 | 자동 진행 (단계 5 측정으로) |
| 단계 4.5 FAIL | 단계 4로 반송, 구조 수정 후 재측정 |
| 차단 게이트 전체 PASS | **자동 커밋** — diff % + 구조 지표 포함, PROGRESS.md 갱신, 즉시 다음 섹션 |
| 3회 실패 | **AI 자체 판단** (재분할→다른접근→엔진차이 수용→되돌리기→완화 순서). 완화 시 `[ACCEPTED_DEBT]` + tech-debt 자동 등록 |

워커 재호출 시 "단계 X부터 진행" 명시.
...
- ✅ v4: 같은 대화 내에서 섹션 연속 처리 **허용** (자율 모드)
