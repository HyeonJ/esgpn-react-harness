...
**v4 구조 게이트 (G5~G8 통과 후 실행)**:
```bash
node scripts/check-structure-quality.mjs --section={섹션명}
```
차단 기준:
- `token_ratio < 0.2` → FAIL (magic number → 디자인 토큰 교체)
- `absolute_count / files > 5` → FAIL (absolute → flex/grid 의미 복원)
- `semantic_score < 2` → 경고만 (차단 아님)
...
| # | 게이트 | 측정 항목 | 통과 기준 | 측정 방법 |
|---|--------|----------|-----------|-----------|
| G2 | 치수 정확도 | font-size, margin, padding, gap | font **±2px**, 나머지 **±4px** | Playwright computed style |
| G4 | 색상 토큰 일치 | 주요 텍스트/배경 색상 | 디자인 토큰 참조 (hex 직접 사용 최소화) | Playwright computed style |

**참고 지표** (수치만 기록, 차단 아님):
| # | 게이트 | 측정 항목 | 목표 | 측정 방법 |
|---|--------|----------|------|-----------|
| G1 | 시각 diff | pixelmatch diff % | **≤ 15%** | `scripts/compare-section.sh {섹션명}` |
| G3 | 에셋 존재 | 모든 `<img>` `naturalWidth > 0` | 전부 통과 | Playwright DOM 검사 |
