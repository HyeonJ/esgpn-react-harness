...
4. `plan/*.md`의 미완료 섹션
5. **Framelink MCP 등록 여부** (`claude mcp list` 또는 세션 도구 목록에 `mcp__figma-framelink__download_figma_images` 존재 여부). 미등록이면 섹션 워커 스폰 전에 사용자에게 `docs/figma-workflow.md` Phase 0 안내 (baseline PNG 저장 불가능하면 G1 게이트 자동 측정이 막힘)
...
| 워커 반환 | 오케스트레이터 행동 |
|----------|------------------|
| 단계 1(리서치) 완료 | approval-gate: 리서치 검토 요청 |
| 단계 2(계획) 완료 | approval-gate: plan 승인 요청 |
| 단계 5 통과 | approval-gate: 커밋 승인 요청 (또는 diff 이미지 재확인 제안) |
| 단계 5 3회 실패 | approval-gate: 사용자 결정 요청 (완화/다른접근/디버깅/되돌리기) |
