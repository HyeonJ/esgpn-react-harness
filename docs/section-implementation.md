...
1. **`get_screenshot`** — 해당 섹션 노드만
   - 저장: `figma-screenshots/{페이지명}/{섹션명}.png` (이미 Phase 2에서 저장됐다면 스킵)
2. **`get_design_context`** — 해당 섹션 노드
   - 토큰 12K 이하 확인. 초과 시 더 작은 노드로 분할 후 재호출
3. (필요 시) **`get_variable_defs`** — 이 섹션에서 처음 등장하는 토큰이 있을 때만
...
2. 대신 **해당 노드의 부모 컨테이너 노드 ID**로 `get_screenshot`을 호출해서 **정적 PNG 한 프레임**을 export
   - 이유: 부모 컨테이너 단위로 떠야 캔버스에서 보이는 합성 결과(오버레이/마스크 포함)가 그대로 들어옴
   - 비디오 노드만 단독으로 캡처하면 위에 얹힌 정적 오버레이가 빠질 수 있음
3. export된 PNG를 `src/assets/{섹션명}/`에 저장하고 코드에서 사용
4. 에셋 목록 표의 "처리 방식" 칸에 "정적 프레임 추출 (get_screenshot, 부모 노드 {ID})"으로 기록
...
- [ ] Figma 베이스라인 스크린샷이 `figma-screenshots/`에 존재
...
```
이 스크립트가 다음을 모두 수행해야 한다:
1. dev 서버에서 해당 섹션 URL 캡처
2. `figma-screenshots/{페이지명}/{섹션명}.png`와 pixelmatch
3. diff 이미지 생성 → `tests/visual/diff/{섹션명}.png`
4. diff % 출력
