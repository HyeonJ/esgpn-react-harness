...
```
[1]   섹션 리서치       → research/{섹션명}.md           ← 통보만
[2]   섹션 계획         → plan/{섹션명}.md               ← 사용자 승인 필요 (real gate)
[3]   에셋 수집         → src/assets/{섹션명}/
[4]   구현              → src/...
[4.5] 품질 게이트       → G5 시맨틱 / G6 텍스트:이미지 / G7 Lighthouse / G8 i18n
                        → 실패 시 단계 4로 반송
[5]   측정 (G1~G4)      → plan/{섹션명}.md 측정 섹션
[6]   수정 루프         → 최대 3회 (실패 시 선택지: 재분할→다른접근→엔진차이 수용→되돌리기→완화)
[7]   자동 커밋         → 모든 게이트 PASS 시 오케스트레이터가 자동 실행
```

**측정 게이트 분리 배치 이유**: G5~G8(구조·품질)을 단계 4.5에 먼저 두어 **구조 망가진 코드가 픽셀 측정에 도달하지 못하게** 막는다. main-hero 카드 raster 사태(4 게이트 모두 PASS로 통과한 안티패턴)를 구조적으로 차단.
...
### 2.1 Figma REST API / MCP 호출 (F-015 이후)

> **핵심 규칙 (리서치 근거)**: Figma REST `/v1/images`는 전달받은 `nodeId`를 **렌더 루트**로 삼는다. 부모 Group/Frame nodeId를 넘기면 자식(텍스트·배경·blur 포함) 전체를 **단일 composite PNG 한 장**으로 받는다. 텍스트 baked-in raster 안티패턴의 90%는 이 호출 실수에서 발생.
>
> **→ 아이콘/장식 raster export 시**: 부모 카드/프레임 nodeId **절대 금지**. **자식 leaf nodeId(아이콘/사진/blur 레이어 등)**만 전달. 자식 nodeId 모르면 `curl GET /v1/files/{key}?ids=<parent>&depth=3`로 트리 탐색 선행 (또는 공식 MCP `get_metadata` 1회).
>
...
1. **`scripts/figma-rest-image.sh`** (Figma REST Images API 래퍼) — 해당 섹션 노드를 baseline PNG로 **파일 저장**
   - 사용: `scripts/figma-rest-image.sh <fileKey> <nodeId> <output-path> --scale 2`
   - 결과 경로 규약:
   ...
   - `FIGMA_TOKEN` 미설정이면 Phase 0 먼저 수행 (`docs/figma-workflow.md §0.2`)
2. **`get_design_context`** (공식 MCP) — 해당 섹션 노드. **섹션당 0~1회만** (Starter 월 6 tool call 쿼터 보호)
   - 토큰 12K 이하 확인. 초과 시 더 작은 노드로 분할 후 재호출
   - 쿼터 부족 시 대체: `curl GET /v1/files/{key}/nodes?ids=<nodeId>` (raw 노드 데이터, 코드 힌트 없음)
3. (필요 시) **`get_variable_defs`** (공식 MCP) — 이 섹션에서 처음 등장하는 토큰이 있을 때만 (페이지당 1회 권장)
4. (참고) **`curl GET /v1/files/{key}?ids=<nodeId>&depth=N`** — 레이아웃/fills/effects가 JSON으로 필요할 때 보조 사용 (쿼터 무관)

**Framelink MCP 절대 호출 금지** (F-015 영구 폐기). `mcp__figma-framelink__*` 어떤 이름이든 호출 금지.
...
2. 대신 **해당 노드의 부모 컨테이너 노드 ID**로 `scripts/figma-rest-image.sh`를 호출해서 **정적 PNG 한 프레임**을 저장
   - 명령: `scripts/figma-rest-image.sh <fileKey> <부모 노드 ID> src/assets/{섹션명}/{이름}-static.png --scale 2`
...
4. 에셋 목록 표의 "처리 방식" 칸에 "정적 프레임 추출 (Figma REST API, 부모 노드 {ID})"으로 기록
...
- [ ] Figma 베이스라인 스크린샷이 **`figma-screenshots/{섹션명}.png`** (flat 경로, Figma REST API로 저장)에 존재
- [ ] **변형(transform) 가진 요소가 있다면 rotation·transform-origin·position을 소수점 포함 원본값 그대로 research에 기록** (반올림 금지 — 아래 §2.4 참조)

### 2.4 정밀 수치 규칙 (반올림 금지)

> **🤖 자동 가드 있음**: `scripts/check-tailwind-antipatterns.sh`가 음수 width/height(ERROR), 정수 회전(`rotate-[Ndeg]`)·정수 letter-spacing(`tracking-[Npx]`)를 WARN로 검출. CI/단계 4.5에서 자동 실행.
...
**연관 원칙:**
- `get_design_context` 또는 REST `/v1/files/{key}/nodes`가 반환하는 수치는 소수점이 붙어있으면 그대로 기록
...
- **Figma REST export SVG에 `scaleY/scaleX` 반전 transform 금지**: `get_design_context`가 `-scale-y-100`/`-scale-x-100` 같은 반전 transform을 명시하더라도, 그건 Figma **내부 저장 형식** 기준 지침이지 Figma REST `/v1/images`가 export한 SVG 기준이 아니다. Figma는 **display-ready 방향**으로 SVG를 저장하므로 추가 CSS 반전 적용 시 **실제 표시 방향이 뒤집힌다** (pixelmatch 수치상 차이 작지만 **시각적으로 명백한 오류**). main-intro 회차 3에서 Vec6/Vec7 커넥터 방향 반전 확인됨

### 2.5 Figma REST PNG는 "완성된 합성 사진"이다 (중요)

> **🤖 자동 가드 있음**: `scripts/check-baked-in-png.sh`가 `src/assets/*.png` import 경로 추적 후 해당 변수 사용처에서 `rotate-[`/`mix-blend-`/`bg-[#` 클래스 중첩 시 경고. 단계 4.5에서 자동 실행.

**Figma REST Images API (`GET /v1/images`)가 반환하는 PNG는 항상 AABB 기준 최종 합성본이다.** 회전·`mix-blend-mode`·배경 채움·알파·그림자 등이 **모두 이미지 자체에 baked-in**된 상태로 나온다. 즉 "**이미 완성된 사진**"으로 취급해야 한다.
...
<div style={{ width: aabbWidth, height: aabbHeight }}  /* Figma metadata AABB */
  ...
  <img src={png}
       width={nativeWidth}         /* Figma REST API가 반환한 실제 PNG native 크기 */
       height={nativeHeight}
       className="block" />
</div>
```
...
- **Figma REST API PNG native 크기 ≠ Figma metadata AABB** (차이 예: leaf-top metadata 166.048 vs native 154.5)
- Figma metadata는 canvas 상 bbox (레이아웃 공간 점유)
- Figma REST API PNG는 실제 렌더된 픽셀 (AABB 안의 유효 영역)
...
**과거 Framelink 파라미터 함정 — 현재는 REST API 단일 경로로 무관 (main-programs-card3 교훈, 참고용)**:
- Framelink에서 `nodeId`만 주기 → **AABB 기준 rendered composite PNG** (정답)
- `imageRef` + `needsCropping: true` → **4096 raw JPEG** (cropped 아님, 함정)
- **F-015 이후**: `scripts/figma-rest-image.sh`가 항상 `GET /v1/images?ids=<nodeId>` 방식만 사용 → AABB composite PNG 고정. 이 함정은 재발 불가.

**반복 SVG 인스턴스 합성 주의**:
- Figma에서 같은 SVG가 반복 배치된 경우(예: card3의 arrow-chevron 3중첩), Figma REST API가 **합성 상태 그대로 export**할 수 있음 (64×32 한 장으로 3개가 이미 중첩된 SVG)
...
**Figma REST API 다운로드 파일명 ≠ 디자인 내용** (main-gallery 교훈):
- design_context가 준 변수명(예: `imgFrame2043685961` → 추정 "cityscape") vs **실제 파일 내용**이 다를 수 있음
- Figma REST API 파일명은 노드 순서 기반이라 디자이너가 의도한 역할과 무관
...
- **우회 전략 모두 막힘**:
  - baseline 재export (Figma REST API nodeId-only) → baseline 자체는 Figma blend 적용된 PNG지만 capture는 여전히 Chromium blend → 엔진 차이 유지
...
**한글 텍스트 다수 + blend 모드 섹션은 G1 6~7% 수렴 패턴** (contest-benefits 교훈):
- contest-benefits 4회차 시도(9.48 → 12.21 → 6.90 → 6.71%) 결과 **6.7% 수렴** 확인
- 잔여 diff 주원인: **Chromium vs Figma Pretendard 폰트 AA 서브픽셀 차이** — diff 이미지 전면에 텍스트 윤곽 빨간선
- 우회 시도 효과:
  - [B] CTA raster composite만: 9.48% → 6.90% (-2.58%p, 주 기여)
  - [C] 카드 아이콘 nodeId-only rendered (imageRef+cropTransform에서 전환): 6.90% → 6.71% (-0.19%p, 미미)
  - [B+C] 결합이 현실적 최소치
- **한글 텍스트 밀도 높은 섹션 + blend 모드 = 6~7% 수렴 패턴**을 기대값으로 설정
- 2회차 초과 실패 시 "코드 문제 아님"으로 빠르게 판정하고 완화 고려 (3회차까지 소진 전)
- 이 패턴이 자격검정·뉴스 목록 등 텍스트 많은 페이지에서 재등장하면 동일 수용

### 2.7 텍스트 baked-in raster 절대 금지 (안티패턴)

> **🤖 자동 가드 있음**: G6(텍스트:이미지 비율) + G8(i18n 가능성)이 `scripts/check-text-ratio.mjs`로 단계 4.5에서 자동 검출. alt 총량 80자 이상 + ratio < 3:1 → FAIL → 단계 4로 반송.

**deep-research 교훈**: G1 단일 지표 최적화로 "텍스트 포함 composite PNG"를 받아 `<img alt="...">` 하나로 렌더하는 것은 **측정 통과 + 실질 가치 희생**의 전형적 안티패턴.

**안티패턴의 정의** — "pixel-perfect 고집 = 이미지로 회귀 = SEO 손실"
- 단기 지표(G1)는 개선됨
- 장기 비용 폭발:
  - **SEO**: 검색엔진이 텍스트 색인 불가
  - **접근성**: 스크린리더 alt 한 줄만 인식
  - **i18n**: 다국어 확장 시 Figma 재디자인 + 재export 필요
  - **유지보수성**: 텍스트 수정 한 글자도 디자이너 의존
  - **사용자 UX**: Ctrl+F 검색 불가, 텍스트 선택 불가, 확대 시 흐려짐

**판정 기준**:
```
이미지 안에 사용자가 읽는 텍스트가 있나?
├─ YES → 텍스트는 HTML로 반드시 재구성
│        (배경·장식만 raster, 텍스트는 HTML 오버레이)
└─ NO  → raster OK (배경/아이콘 등)
```

**✅ raster 허용 대상** (텍스트 없는 시각 요소만):
- 배경 이미지, 파노라마 사진 (about-vision panorama, main-hero bg)
- 장식 사진 (about-mission photo, main-gallery cityscape)
- 아이콘·로고 (contest-about 지구본, about-organization-logos ESPGN/Colive)
- 벡터 장식 (dividers, arrows, dots)
- blend 효과 baked-in 배경 (**텍스트 제외**)

**❌ raster 금지 대상** (텍스트 포함):
- 카드 내부 텍스트 + 배경 합성
- CTA 라벨 + 배경 합성
- 제목·본문 + 장식 합성
- 어떤 형태든 text-bearing composite

**G1 vs 접근성 충돌 시 우선순위**:
1. **텍스트 HTML 유지는 non-negotiable** (G1 상관없이)
2. G1이 7~10% 나와도 HTML 유지 + 완화 사유 기록
3. 완화는 "blend 엔진 차이"·"폰트 AA 서브픽셀" 같은 구조적 한계만 정당화

**기존 위반 사례 (기술 부채로 백로그)**:
- `main-hero` 카드 3개 — 카드 전체 raster (텍스트 baked-in). 별건 리팩터 PR 대기
- `contest-benefits` CTA — "지금 바로 신청하세요" 텍스트 baked-in composite. 별건 리팩터 PR 대기

### 2.6 baseline PNG 실제 크기 측정 (Figma spec과 다를 수 있음)

Figma REST API가 반환한 baseline PNG 크기가 **Figma 노드 spec 치수와 다를 수 있다** (예: spec 1416 vs 실제 1404 — 좌측 트림).
...
## 5. 단계 4 — 구현 + 단계 4.5 품질 게이트

### 단계 4.5: G5~G8 품질 게이트 (단계 5 진입 전 필수)

구현(단계 4) 직후 **G1~G4 측정 전에** G5~G8을 먼저 측정. 구조 망가진 코드가 픽셀 측정 단계에 도달하지 못하게 한다.

```bash
# 한 번에:
bash scripts/measure-quality.sh {섹션명} src/components/sections/{섹션디렉}

# 개별:
npx eslint src/components/sections/{섹션디렉}                  # G5
node scripts/check-text-ratio.mjs src/components/sections/{섹션디렉}  # G6/G8
# G7 Lighthouse — dev 서버 기동 + @lhci/cli 설치 시 측정 가능

# 자동 가드 경고 (참고):
bash scripts/check-tailwind-antipatterns.sh src/components/sections/{섹션디렉}
bash scripts/check-baked-in-png.sh {섹션명}
```

**G5~G8 통과 조건**
- G5: eslint jsx-a11y error 0
- G6: ratio ≥ 3:1 (alt 총 80자 이상일 때만 적용)
- G7: Lighthouse a11y ≥ 95, SEO ≥ 90 (lhci 설치 시)
- G8: JSX에 literal text 존재 (alt/aria 제외)

**실패 시 대응**
- G5 FAIL: eslint 에러 수정 (landmark 태그, role 교정)
- **G6 FAIL: 텍스트 baked-in raster 강한 의심 → HTML 재구성. 완화 금지**
- G7 FAIL: Lighthouse 개별 audit 수정 (alt, heading 순차성, contrast)
- G8 FAIL: 컴포넌트에 literal text 추가

단계 4.5 통과 전까지 단계 5 진입 금지. 실패 → 단계 4로 반송.
...
- Figma REST API baseline PNG가 **alpha=0 투명 픽셀을 white로 blend**하는 렌더러 기본 동작 때문
...
2. **`figma-screenshots/{섹션명}.png`**(flat, Figma REST API 저장 경로)와 pixelmatch
...
|---------|------------|------------|----------|
| 방향 반전 (SVG flip) | 작음 (0.05%p 수준) | **명백한 시각 오류** | `-scale-y-100`을 pre-flipped SVG에 이중 적용 |
...
### 6.5 페이지 통합 육안 게이트 (필수)

**격리 Preview에서 PASS해도 실제 사용자 라우트에서 깨질 수 있다.** Preview는 `__preview/{섹션}` 라우트에 wrapper를 씌워 캡처하지만, 실제 라우트(`/`, `/about`, `/contest` 등)는 RootLayout 안에 섹션을 연속 마운트한다. 다음 류의 **조립 회귀는 Preview에서 안 보인다**:

| 회귀 유형 | Preview에서 | 실제 라우트에서 | 원인 예시 |
|---------|-------------|---------------|----------|
| 좌측 정렬 | 중앙정렬 (wrapper `mx-auto`) | **좌측 치우침** | 섹션 루트 `mx-auto` 누락 |
| 가로 스크롤 | 없음 (clip) | **1920 초과 요소 유출** | 음수 left, 절대 포지션 자식 초과 |
| Sibling margin collapse | 없음 (단독) | 섹션 간 간격 변형 | 인접 섹션 margin 상쇄 |
| z-index 충돌 | 없음 (단독) | Header/Footer 가림 | 섹션 내 fixed/sticky |
| 중복 배경 | 단독 배경 | **이중 배경** | RootLayout이 이미 bg 지정 |

**검증 절차 (페이지 내 새 섹션이 마운트될 때마다 수행, 또는 페이지 조립 직후 1회):**

1. 실제 라우트(`/contest` 등) 1920 viewport fullPage 캡처
2. 캡처를 육안으로 스캔:
   - 각 섹션이 기대한 수평 정렬인가? (풀폭 1920 vs 중앙정렬 1416)
   - 가로 스크롤 유발 없나?
   - 섹션 간 간격이 Figma 의도와 맞나?
   - Header/Footer가 의도대로 겹치는가?
3. 회귀 발견 시 해당 섹션 **단계 6 재진입**. plan에 "페이지 통합에서 X 회귀 발견, N회차에서 수정" 기록
4. PROGRESS.md 섹션 완료 줄에 `/ 페이지통합 PASS` 표기 추가

**원칙: 섹션이 자기 정렬 책임을 진다.** Preview wrapper로 정렬을 해결하면 실제 라우트와 어긋난다. 1416 너비 섹션은 섹션 루트에 `mx-auto`, 1920 풀폭 섹션도 루트에 `mx-auto`(중앙 fallback). Preview 라우트는 섹션을 그대로 렌더 (`<div className="w-[1416px] mx-auto bg-white">` 같은 wrapper 금지).

**워커 책임:** 섹션 완료 보고 전 페이지 통합 캡처 1장 확인. 오케스트레이터는 페이지의 첫 섹션 이후 모든 후속 섹션에서 이 게이트 결과를 확인.
...
**임의로 우회하지 않는다.** 선택지 순서는 **재분할 → 다른접근 → 엔진차이 수용 → 되돌리기 → 완화(최후)**.

- `[A] 재분할`: 이 섹션이 너무 크다는 시그널. 이질 에셋 3+ / 반복 자식 3+ 있으면 서브섹션으로 쪼개기 먼저. Phase 2로 부분 복귀
- `[B] 다른 접근`: 구현 전략 교체 (raster → HTML, blend 모드 변경 등)
- `[C] 엔진 차이 수용`: hard-light blend, 한글+blend 6~7% 수렴 등 구조적 불가피 → `docs/tech-debt.md`에 `ACCEPTED` 등록 (부채 카운트에서 제외)
- `[D] 되돌리기`: 섹션 처음부터 재시작
- `[E] 완화 (최후)`: G1/G4 같은 픽셀/색상만 허용. **G5~G8 품질 게이트는 완화 불가**. 선택 시:
  - 커밋 메시지 `[ACCEPTED_DEBT]` 태그 강제
  - `docs/tech-debt.md`에 `OPEN` 엔트리 자동 append
  - `OPEN` 부채 3건 도달 시 다음 섹션 진행 차단

사용자에게 다음 형식으로 보고하고 멈춘다:
...
모든 게이트(G1~G8) + 육안 PASS 시 오케스트레이터가 **자동 실행**. 사용자 승인 게이트는 섹션 단계 2(plan)에만 있다.

### 8.1 자동 커밋 조건
- [ ] G1~G4 모두 통과 (측정값 plan에 기록)
- [ ] G5~G8 모두 통과 (측정값 plan에 기록)
- [ ] 육안 semantic 검증 통과
- [ ] 페이지 통합 육안 게이트 통과
- [ ] 빌드/린트/타입 체크 통과
- [ ] 완화(ACCEPTED_DEBT) 있는 경우 `docs/tech-debt.md` 엔트리 생성 완료
...
# 완화 커밋
git commit -m "feat(section): {페이지}-{섹션명} 구현 (diff X.X% ⚠ 완화) [ACCEPTED_DEBT] [auto]"
```

diff % + G5-G8 상태 포함. 나중에 git log로 품질 추적 가능.
