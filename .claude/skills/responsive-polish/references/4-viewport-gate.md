...
// 통과 조건 1: scrollX <= clientX + 1  (서브픽셀 여유 1)
```

### A1-b. 실 컨텐츠 잘림 없음 (중요 — A1만으로 불충분)
`detect-cutoff.mjs` 실행 시:
- `h-overflow` (UNCLIPPED) === 0 — 실제 viewport 초과 요소 없어야
- `h-overflow-clipped` 는 허용되되, **실 컨텐츠(텍스트/이미지)를 포함한 요소는 제외** — 장식만 남아야

**clip은 UX 통과가 아니다.** scrollWidth 일치해도 내용이 오른쪽으로 잘려 안 보이면 실패.
